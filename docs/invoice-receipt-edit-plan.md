...
**STEP 4 (FinancialEngine) COMPLETE.** Next: **Step 5 — self-healing triggers** wired to the engine, gated by `Financial_Engine_Settings__c.Engine_Owns_Totals__c`, on Receipt/Receipt_Amount/Receipt_Invoice/Milestone_Invoice/Invoice (insert/update/delete/undelete). Legacy `ReceiptRecordInsertTest`/`InvoiceRecordInsertTest` must be updated to set the flag OFF so they keep testing the old path in isolation (see Step 3 note).
