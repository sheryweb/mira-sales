...
export default class MultiSalesOfferWizard extends NavigationMixin(
  LightningElement
) {
  step = STEP_PROJECT;
  accessDenied = false;
  ...
  get showBody() {
    return this.initLoaded && !this.accessDenied;
  }
  get isStep1() {
    return this.step === STEP_PROJECT;
  }
  get isStep2() {
    return this.step === STEP_UNITS;
  }
  get isStep3() {
    return this.step === STEP_RESULT;
  }
  get step1Class() {
    return this.stepClass(STEP_PROJECT);
  }
  get step2Class() {
    return this.stepClass(STEP_UNITS);
  }
  get step3Class() {
    return this.stepClass(STEP_RESULT);
  }
  stepClass(n) {
  ...
  get unitCards() {
    ...
    return this.units
      .filter(
        (u) => !this.typeFilter || this.bedroomTypeLabel(u) === this.typeFilter
      )
      .map((u) => {
        ...
        return {
          ...
          areaDisplay:
            u.totalArea == null
              ? "—"
              : Number(u.totalArea).toLocaleString("en-US", {
  ...
  get hasUnits() {
    return this.unitsLoaded && this.units.length > 0;
  }
  get selectedCount() {
    return this.selectedIds.size;
  }
  get counterClass() {
    return this.selectedIds.size >= this.maxUnits
      ? "count-chip full"
      : "count-chip";
  }
  ...
  handleUnitToggle(event) {
    ...
    if (next.has(id)) {
      ...
      this.toast(
        "Selection limit",
        `A maximum of ${this.maxUnits} units can be selected in one go.`,
        "warning"
      );
      return;
    } else {
  ...
  handleGenerate() {
    if (this.selectedIds.size === 0) {
      this.toast(
        "Nothing selected",
        "Please select at least 1 unit in order to proceed.",
        "warning"
      );
      return;
    }
  ...
  handleDownload() {
    // The Android Salesforce app cannot navigate to the raw download servlet
    // (nothing happens); iOS only works by accident of its file viewer. On
    // phones/tablets route through the native file preview instead — it has
    // download/share built in and works on both platforms.
